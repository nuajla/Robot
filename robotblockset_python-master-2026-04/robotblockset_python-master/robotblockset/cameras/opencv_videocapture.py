...
This module defines an OpenCV ``VideoCapture`` camera wrapper implementing the RGB camera interface.
It provides a lightweight backend for webcams and video files, with configurable resolution and frame rate,
and conversion utilities for consistent image formats used across robotblockset. The module is primarily
intended for development, testing, and interface validation without specialized camera hardware.

Key functionalities include:
- OpenCV ``VideoCapture`` integration through the ``RGBCamera`` interface.
- Support for webcam devices and video-file input streams.
- Configurable capture resolution and target frame-rate settings.
- RGB image retrieval in float and uint8 formats via shared converters.
- Optional intrinsics matrix support for calibration-aware workflows.
- Context-managed resource handling for clean camera release.
...
if __name__ == "__main__":
    ...
    camera = OpenCVVideoCapture(video_capture_args=(2,), intrinsics_matrix=np.eye(3))
