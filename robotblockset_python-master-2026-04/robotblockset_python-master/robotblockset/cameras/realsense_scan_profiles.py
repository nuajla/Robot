...
def scan_profiles(
    stream_profiles: List[rs.stream_profile],  # type: ignore
    ...
    Parameters
    ----------
    stream_profiles : List[rs.stream_profile]
        RealSense stream profiles to inspect.

    Returns
    -------
    tuple[dict[tuple[int, int], list[float]], dict[tuple[int, int], tuple[float, float]]]
        Two mappings: supported FPS values per resolution and FoV per
        resolution.
    """
...
def print_profile_info(
    ...
    ----------
    resolution_fps_combinations : Dict[Tuple[int, int], List[float]]
        Supported FPS values per resolution.
    resolution_fov_combinations : Dict[Tuple[int, int], Tuple[float, float]]
        Horizontal and vertical FoV per resolution.
    ...
    """
    for resolution in resolution_fps_combinations:
        fov_string = f"({resolution_fov_combinations[resolution][0]:.1f}°, {resolution_fov_combinations[resolution][1]:.1f}°)"
...
for color_resolution, color_fps in color_resolution_max_fps.items():
    for depth_resolution, depth_fps in depth_resolution_max_fps.items():
        ...
        print(f"color_resolution = {color_resolution}, depth_resolution = {depth_resolution}, aligned_depth = ({aligned_depth_frame.get_width()}, {aligned_depth_frame.get_height()})")
        pipeline.stop()
