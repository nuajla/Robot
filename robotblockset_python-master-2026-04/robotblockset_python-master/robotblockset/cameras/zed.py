...
This module defines a ZED stereo RGB-D camera wrapper built on the ZED SDK Python bindings.
It provides stereo image acquisition, depth and point-cloud retrieval, intrinsics access for left/right views,
and runtime configuration for resolution, frame rate, and depth processing modes. The implementation exposes
ZED devices through the shared stereo RGB-D camera interface for unified downstream processing.
...
import cv2
import numpy as np
from robotblockset.cameras.interfaces import StereoRGBDCamera
...
try:
    ...
    raise ImportError("You should install the ZED SDK and pip install the python bindings in your environment first, see the installation README.")

# check SDK version when available; allow mocked imports during docs builds
_sdk_version = sl.Camera().get_sdk_version()
if isinstance(_sdk_version, str) and _sdk_version.split(".")[0] != "5":
...
class Zed(StereoRGBDCamera):
    ...
    def __init__(  # type: ignore[no-any-unimported]
        ...
        resolution : CameraResolutionType, optional
            CameraResolutionType, default=RESOLUTION_2K
            Requested stream resolution ``(width, height)``.
        fps : int, optional
            int, default=15
            Target camera frame rate.
        depth_mode : sl.DEPTH_MODE, optional
            sl.DEPTH_MODE, default=NONE_DEPTH_MODE
            Depth processing mode configured in the ZED SDK.
        serial_number : str, optional
            Camera serial number used to select a specific device.
        svo_filepath : str, optional
            Path to an SVO file used as the camera input source.

        Raises
        ------
        IndexError
            If the camera cannot be opened after multiple attempts.
        """
        ...
        if status != sl.ERROR_CODE.SUCCESS:
            logger.error("Could not open Zed camera. Sometimes, unplugging the camera and plugging it back in helps.\n" "Alternatively, try to reboot the camera by running `ZED_Explorer --reboot` in a terminal.")
    ...
    def intrinsics_matrix(self, view: str = StereoRGBDCamera.LEFT_RGB) -> CameraIntrinsicsMatrixType:
        ...
        ----------
        view : str, optional
            str, default=StereoRGBDCamera.LEFT_RGB
            Requested view identifier.

        Returns
        -------
        CameraIntrinsicsMatrixType
            Rectified camera intrinsics matrix with shape ``(3, 3)``.

        Raises
        ------
        ValueError
            If ``view`` is not a supported stereo view.
        """
    ...
    def depth_enabled(self, value: bool) -> None:
        """
        Enable or disable depth and point-cloud computation at runtime.

        Parameters
        ----------
        value : bool
            Whether depth computation should be enabled.
    ...
    def _grab_images(self) -> None:
        """Grab and process the latest ZED images.
    ...
    def _retrieve_rgb_image(self, view: str = StereoRGBDCamera.LEFT_RGB) -> NumpyFloatImageType:
        """
        Retrieve the latest RGB frame as normalized floating-point image.

        Parameters
        ----------
        view : str, optional
            str, default=StereoRGBDCamera.LEFT_RGB
            Requested stereo view.

        Returns
        -------
        NumpyFloatImageType
            RGB image in float format.
        """
    ...
    def _retrieve_rgb_image_as_int(self, view: str = StereoRGBDCamera.LEFT_RGB) -> NumpyIntImageType:
        """
        Retrieve the latest RGB frame as integer image.
    ...
    def _retrieve_depth_map(self) -> NumpyDepthMapType:
        """
        Retrieve the latest depth map in metric units.

        Returns
        -------
        NumpyDepthMapType
            Depth map relative to the left camera.

        Raises
        ------
        RuntimeError
            If depth mode is disabled or depth retrieval is turned off.
        """
    ...
    def _retrieve_depth_image(self) -> NumpyIntImageType:
        """
        Retrieve the latest colorized depth image.
    ...
    def _retrieve_colored_point_cloud(self) -> PointCloud:
        """
        Retrieve the latest colored point cloud.

        Returns
        -------
        PointCloud
            Point cloud with XYZ positions and RGB colors.
    ...
    def _retrieve_confidence_map(self) -> NumpyConfidenceMapType:
        """
        Retrieve the latest normalized depth confidence map.

        Returns
        -------
        NumpyConfidenceMapType
            Confidence map in ``[0, 1]``, where higher is better.
        """
    ...
    def __exit__(self, exc_type: Any, exc_value: Any, traceback: Any) -> None:
        """
        Exit context manager and close the camera device.
...
def _test_zed_implementation() -> None:
    ...
    from robotblockset.cameras.manual_test_hw import manual_test_stereo_rgbd_camera, profile_rgb_throughput
