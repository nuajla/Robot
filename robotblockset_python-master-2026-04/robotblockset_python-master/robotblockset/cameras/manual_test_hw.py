...
import numpy as np
from robotblockset.cameras.interfaces import Camera, DepthCamera, RGBCamera, RGBDCamera, StereoRGBDCamera
...
def manual_test_depth_camera(camera: DepthCamera) -> None:
    ...
    input("The depth map values should be reasonable and the resolution should match the camera resolution. Press a key if all seems fine.")
...
def manual_test_stereo_rgbd_camera(camera: StereoRGBDCamera) -> None:
    ...
    input("The pose of the right view in the left view should be as expected: a translation along the X-axis with the camera disparity distance. Press a key if all seems fine.")
