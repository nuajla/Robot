...
// communicate in background thread
void CommunicationLoop(mj::Simulate& sim) {
  // run until asked to exit
  while (!sim.exitrequest.load()) {
    // sleep for 1 ms or yield, to let main thread run
    // yield results in busy wait - which has better timing but kills battery life
...
void CommThread(mj::Simulate* sim){

  CommunicationLoop(*sim);
}
...
int main(int argc, char** argv) {
  ...
  // TCP server at localhost
  const char* host = nullptr;
  const char* port = nullptr;
  if (argc > 2) {
      host = argv[1];
      port = argv[2];
      mx_connect(host,port);
  }
  else if (argc > 1) {
      host = argv[1];
      mx_connect(host);
  ...
  std::thread commthreadhandle(&CommThread, sim.get());
