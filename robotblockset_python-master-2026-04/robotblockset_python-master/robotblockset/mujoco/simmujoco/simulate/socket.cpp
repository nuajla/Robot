...
mjSocket::mjSocket()
{
	// clear
	soc = INVALID_SOCKET;
	listen_soc = INVALID_SOCKET;
	state = false;
	listen_state = false;
	_errorfunc = 0;

	// default settings
	//strcpy(portListen, "50000");
	portListen[0] = 0;
	hostListen[0] = 0;
	verbose = false;
...
{
	if( verbose )
		printf("Socket object destruction\n");
	clear();
	listen_clear();
}
...
bool mjSocket::connectServer(int tmout, bool print, const char* hostServer, const char* hostPort)
{
	// what if we are already connected ???

	// get address info for local machine
	if (hostServer) {
		strcpy(hostListen, hostServer);
	}
	else {
		strcpy(hostListen, "localhost");
	}
	// get port info for local machine
	if (hostPort) {
		strcpy(portListen, hostPort);
	}
	else {
		strcpy(portListen, "50000");
	}
	if( verbose )
		printf("Connecting server (%s:%s)\n",hostListen,portListen);
	struct addrinfo* info = getHost(true, portListen, hostListen);
	if( !info )
	...
	listen_soc = socket(info->ai_family, info->ai_socktype, info->ai_protocol);
	if( listen_soc == INVALID_SOCKET )
		_error("socket creation failed with error %d", mjSocketError());

	// get rid of reuse error
	int on = 1;
	if( setsockopt(listen_soc, SOL_SOCKET, SO_REUSEADDR, (char*)&on, sizeof(int)) )
		_error("setsockopt failed with error %d", mjSocketError());

	// bind, release addrinfo
	if( bind(listen_soc, info->ai_addr, (int)info->ai_addrlen) )
		_error("socket bind failed with error %d", mjSocketError());
	freeaddrinfo(info);

	// set keepalive and non-blocking
	if( setsockopt(listen_soc, SOL_SOCKET, SO_KEEPALIVE, (char*)&on, sizeof(int)) )
		_error("setsockopt failed with error %d", mjSocketError());
	mjSetBlocking(listen_soc, false);

	// start listening
	if( listen(listen_soc, SOMAXCONN) )
...
// accept connection
bool mjSocket::acceptClient(int tmout)
{
	bool res=waitSocket(listen_soc, true, tmout);
	if( res )
	{
		if( verbose )
			printf("Accepting (%d) (S:%d-%d) (L:%d-%d) ... \n",(int)res,(int)soc,state,(int)listen_soc,listen_state);
		int on = 1;
		// accept connection
		if ( !state ) {
			struct sockaddr_in client_address;
			socklen_t client_address_len = sizeof(client_address);
			soc = accept(listen_soc,(struct sockaddr *)&client_address, &client_address_len);
			...
			if( verbose )
				printf("Accepted  (S:%d-%d) (L:%d-%d) ... \n",(int)soc,state,(int)listen_soc,listen_state);
			listen_clear();
		}
		else 
		{
			if( verbose )
				printf("Already connected (%d) (S:%d-%d) (L:%d-%d) ... \n",(int)res,(int)soc,state,(int)listen_soc,listen_state);
		};
	}
	return state;
}
...
void mjSocket::clear(void)
{
	...
	soc = INVALID_SOCKET;
	if ( !listen_state ) {
		if ( connectServer(5000, 1, hostListen, portListen) )
		{
			if (verbose)
				printf("Server listening (S:%d L:%d) ... \n",(int)soc,(int)listen_soc);
		}
		else
		{
			if (verbose)
				printf("Server NOT listening (S:%d L:%d) ... \n",(int)soc,(int)listen_soc);
		}
	}
}
...
// close listen socket
void mjSocket::listen_clear(void)
{
	if( state && closesocket(listen_soc) )
		if( verbose )
			printf("close listen socket error %d", mjSocketError());
	listen_state = false;
	listen_soc = INVALID_SOCKET;
	if( verbose )
		printf("Listen socked closed (S:%d-%d) (L:%d-%d)\n",(int)soc,state,(int)listen_soc,listen_state);
}
	...
	while( *received<len )
	{
		...
		if( waitSocket(soc, true, *received ? tmout2 : tmout1) )
		{
			...
			if( n<=0 )
			{
				int err = (n==0) ? 2 : mjSocketError();
				if( verbose )
					printf(" error in recvBuffer (immediate): %d\n", err);
				clear();
				return err;
			}
			else if( verbose )
...
struct addrinfo* mjSocket::getHost(bool passive, const char* port, const char* host)
{
	...
	gethostname(autoname, 100);
 	if( verbose )
		printf("Host autoname: %s Used host: %s\n", autoname,(host[0] ? host: autoname));    
	if( getaddrinfo((host[0] ? host: autoname), port, &hints, &info) )
		return 0;
	else
		return info;	
}
