...
public:
	...
	bool connectServer(int tmout = -1, bool print = false, 
					   const char* hostServer = 0, const char* portServer = 0);

	// accept connection
	bool acceptClient(int tmout = -1);

	// clear socket
	void clear(void);

	// clear listen socket
	void listen_clear(void);
	...
	bool getListenState(void)	{return listen_state;}
	...
	char hostListen[100];			// host for listening (user can change the default)
	...
	SOCKET	listen_soc;				// socket object

private:
	bool	state;					// last known state (true: connected)
	bool 	listen_state;			// last known state (0: listening 1: connected)
