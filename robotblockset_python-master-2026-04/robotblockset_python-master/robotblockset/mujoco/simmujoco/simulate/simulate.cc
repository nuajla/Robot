...
void UpdateInfoText(mj::Simulate* sim, const mjModel* m, const mjData* d,
  ...
  // prepare info text
  char remote[10];
  strcpy(remote,(mx_connected() ? "Connected" : (mx_available() ? "Waiting" : "Disabled")));
...
void Simulate::Render() {
  ...
  // show server message
  if (show_message[0]!=0) {
     mjr_overlay(mjFONT_NORMAL, mjGRID_TOPLEFT, rect, show_message, 0,
                &this->platform_ui->mjr_context());
  }
  ...
  while (!this->platform_ui->ShouldCloseWindow() && !this->exitrequest.load()) {
    {
      ...
      // load model mujoco_server
      if (load_filename[0]!=0) {
        printf("Model file: %s: \n",load_filename);
        mju::strcpy_arr(this->filename, load_filename);
        load_filename[0] = 0;
        this->uiloadrequest.fetch_add(1);
      }
